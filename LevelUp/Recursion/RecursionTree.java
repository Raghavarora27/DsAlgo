...
public class RecursionTree {
    ...
    public static int singlePermutation_subseq(int[] coins, int idx, int tar, boolean[] vis, String asf) {
        if (tar == 0 || idx >= coins.length) {
            if (tar == 0) {
                System.out.println(asf);
                return 1;
        ...
        if (!vis[idx] && tar - coins[idx] >= 0) {
            vis[idx] = true;
            count += singlePermutation_subseq(coins, 0, tar - coins[idx], vis, asf + coins[idx] + " ");
            vis[idx] = false;
        }
        count += singlePermutation_subseq(coins, idx + 1, tar, vis, asf);
    ...
    public static void coinchange() {
        ...
        System.out.println(singlePermutation_subseq(coins, 0, tar, vis, asf));
    ...
    // so far
    public static int queenCombination1D(int tnb, int bno, int tnq, int qpsf, String asf) { // Single Combination
        if (qpsf > tnq) {
            System.out.println(asf);
            return 1;
        }
    ...
    public static int queenCombination1D_sub(int tnb, int bno, int tnq, int qpsf, String asf) {
        // ek baar queen place hogi ,ek baar nhi hogi
        if (qpsf > tnq || bno > tnb) {
            if (qpsf > tnq) {
                System.out.println(asf);
                return 1;
    ...
    public static int queenPermutation1D(int tnb, int tnq, int qpsf, boolean[] vis, String asf) {
        // har queen ko sare boxes ke option diye jayenge , bas vo boxes restrict rhenge
        // jisme queen phele se baithi h
        if (qpsf > tnq) {
            System.out.println(asf);
            return 1;
        }
    ...
    public static int queenCombination2D_sub(int tnb, int bno, int tnq, int qpsf, String asf) {
        ...
        // count += queenCombination2D_sub(tnb,bno + 1,tnq, qpsf - 1,asf + "(" + r + ","
        // + c + ") ");
