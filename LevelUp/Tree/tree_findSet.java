...
public class tree_findSet {
  ...
  public static int maximum(TreeNode root) {
    return root == null
      ? -(int) 1e9
      : Math.max(Math.max(maximum(root.left), maximum(root.right)), root.val);
  }

  public static int minimum(TreeNode root) {
    return root == null
      ? (int) 1e9
      : Math.min(Math.min(minimum(root.left), minimum(root.right)), root.val);
  }
  ...
  public static ArrayList<Integer> kaway(
    TreeNode root,
    int k,
    TreeNode target
  ) {
  ...
  public static void burningTreeNode(
    TreeNode root,
    int time,
    TreeNode blockNode,
    ArrayList<ArrayList<Integer>> ans
  ...
  public static boolean NodeToRootPath_(
    TreeNode root,
    int data,
    List<TreeNode> ans
  ) {
    ...
    boolean res =
      NodeToRootPath_(root.left, data, ans) ||
      NodeToRootPath_(root.right, data, ans);
  ...
  public static boolean lowestCommonAncestor_(
    TreeNode root,
    TreeNode p,
    TreeNode q
  ) {
    ...
    if (
      ...
      (selfPresent && rightPresent) ||
      (selfPresent && leftPresent)
    ) LCANode = root;

    return leftPresent || rightPresent || selfPresent;
  }

  /// try using preorder morris traversal
